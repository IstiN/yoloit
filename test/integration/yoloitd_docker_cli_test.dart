...
void main() {
  ...
  test(
    'real yoloit CLI drives yoloitd through remote connection',
    () async {
      final server = await YoloitdDockerHarness.start();
      addTearDown(server.dispose);

      final cli = await YoloitCliHarness.create(
      ...
      final created = await cli.json(['board:create', 'CLI Docker Board']);
      ...
      final names =
          (boards['boards'] as List<dynamic>)
              ...
              .toList();
      expect(names, contains('CLI Docker Board'));

      final renamed = await cli.json(
        ['board:rename', 'CLI Docker Board', 'Renamed CLI Board'],
      );
      ...
      final noteSet = await cli.json(
        ['do', 'Renamed CLI Board', 'Notes v2', 'set', '{"text":"Hello from CLI"}'],
      );
      ...
      final boardsFinal = await cli.json(['boards']);
      final finalNames =
          (boardsFinal['boards'] as List<dynamic>)
              .whereType<Map<String, dynamic>>()
              .map((board) => board['name'])
              .toList();
      expect(finalNames, contains('Renamed CLI Board'));
    ...
    skip:
        runDocker
            ? false
            : 'Set YOLOIT_RUN_DOCKER_TESTS=1 to run Docker CLI tests.',
