...
class _EditorCardState extends State<EditorCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final editorBody = _buildEditorBody(colors, props, lines);
        ...
        child: Column(
          ...
          children: [
            if (!widget.immersive &&
                widget.body == null &&
                props.tabs.length > 1)
              _buildTabBar(colors, props),
            if (!widget.immersive && widget.body == null)
              _buildHeader(colors, props),
  ...
  BoxDecoration _cardDecoration(AppColorScheme colors) {
  ...
  Widget _buildEditorBody(
    ...
    EditorCardProps props,
    List<String> lines,
  ) {
    if (props.isImage) {
      return _ImageBody(base64: props.imageBase64!, filePath: props.filePath);
    }
    if (_isEditing && widget.onContentUpdate != null) {
      return _buildEditingBody(colors);
    }
    if (_isPreview && _isMarkdown) {
      return _buildPreviewBody(colors, props);
    }
    return _buildCodeBody(colors, lines);
  }

  Widget _buildEditingBody(AppColorScheme colors) {
  ...
  Widget _buildPreviewBody(AppColorScheme colors, EditorCardProps props) {
    return Container(
  ...
  Widget _buildCodeBody(AppColorScheme colors, List<String> lines) {
    return Container(
      color: colors.terminalBackground,
      ...
      child: RepaintBoundary(
        child: SelectionArea(
          child: ListView.builder(
            ...
            itemBuilder: (_, i) => _buildCodeLine(colors, lines, i),
          ),
        ),
      ),
    );
  ...
  Widget _buildCodeLine(AppColorScheme colors, List<String> lines, int i) {
    return Row(
  ...
  Widget _buildTabBar(AppColorScheme colors, EditorCardProps props) {
    return Container(
      height: 28,
  ...
  Widget _buildHeader(AppColorScheme colors, EditorCardProps props) {
    return Container(
  ...
  Widget _buildEditButton(EditorCardProps props) {
    return _HeaderButton(
