...
part 'cli_server_panel_update_helpers.dart';
...
class CliServer {
  ...
  Future<shelf.Response> _updatePanel(
    ...
    await _applyPanelTitleUpdate(cubit, board, panel, body);
    await _applyPanelMoveUpdate(cubit, board, panel, body);
    await _applyPanelResizeUpdate(cubit, board, panel, body);
    await _applyPanelFocusUpdate(cubit, board, panel, body);
    await _applyPanelColorUpdate(cubit, board, panel, body);
    await _applyPanelHiddenUpdate(cubit, board, panel, body);
    await _applyPanelZIndexUpdate(cubit, board, panel, body);
    return cliJson({'ok': true});
  }
  ...
  Future<shelf.Response> _panelAction(
    ...
    final actionArgs = _resolvePanelActionArgs(cubit, board, panel, body);
    final result = await handler.handleAction(action, actionArgs, panel);

    await _applyPanelActionResult(cubit, board, panel, result);
  ...
  Future<shelf.Response> _applyYaml(
    ...
    for (var i = 0; i < operations.length; i++) {
      ...
      results.add(result);
      if (result['ok'] != true) {
        return _yamlOperationFailure(i + 1, result, results);
      }
      currentBoard = _yamlBoardAfterOperation(
        currentBoard,
        opMap,
        result,
        pendingPanels,
  ...
  Future<void> _applyYamlPanelUpdates(
    ...
    await _yamlApplyPanelTitle(cubit, board, panel, updates);
    await _yamlApplyPanelPosition(cubit, board, panel, updates);
    await _yamlApplyPanelSize(cubit, board, panel, updates);
    await _yamlApplyPanelHidden(cubit, board, panel, updates);
    await _yamlApplyPanelAttributes(cubit, board, panel, updates);
